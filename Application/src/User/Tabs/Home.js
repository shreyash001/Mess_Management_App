...
const Home = () => {
  ...
  const handleLeaveBtn = async () => {
    ...
    const leaveDoc = await firestore()
      .collection('Leave')
      ...
      if (toggleCheckBox1 || toggleCheckBox2) {
        ...
        if (userIndex !== -1) {
          await firestore()
            ...
            .doc(date.toDateString())
            .update({
              leaveUsers: firestore.FieldValue.arrayRemove(
                leaveUsers[userIndex],
              ),
            });
        }
        Alert.alert(
          'Invalid Request',
          `No leave data found, Deleting ${userId} Leave from List`,
        );
      }
    } else {
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.itemView}>
        <View style={styles.orderView}>
          <Text style={styles.nameText}>Home 2</Text>
          <ViewButton
            text="View"
